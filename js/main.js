...
window.onload = function() {
	...
	const promo = new HystModal({
		linkAttributeName: "data-hystmodal-promo",
		catchFocus: false,
	});
	promoModal = function () {
		modals.open('#promo')
	...
	let copyTrigger = document.querySelector('.modal__copy');
	let copyMessage;
	let promoCode = document.getElementById('code');
	let selection = window.getSelection();
	let range = document.createRange();
	let showNotification = function () {
		copyMessage = notifier.show('', 'Промокод скопирован', '', '', 2000);
	};
	let showNotificationError = function () {
		copyMessage = notifier.show('', 'Ошибка', '', '', 2000);
	};
